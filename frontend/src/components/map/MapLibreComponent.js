...
export default function MapLibreComponent() {
  ...
  // --- Map instance for overlays (MapLibre) ---
  const map = mapRef.current?.getMap?.();

  // Insert before the first text-symbol layer so street labels stay above the route
  const labelLayerId = useMemo(() => {
    if (!mapLoaded || !map) return undefined;
    const layers = map.getStyle()?.layers || [];
    const firstSymbol = layers.find(l => l.type === 'symbol' && l.layout && l.layout['text-field']);
  ...
  const layers = useMemo(() => {
    ...
    return uniq;
  }, [bboxEditableLayer, debugRouteLine, debugRouteDots, waypointLayer, geoLayers, lassoLayer, dragLayer, lastBboxLayer, etaTextLayer, tripsLayer]);
  ...
  return (
    <>
      ...
      >
        ...
        {/* Draw the styled route on the MapLibre map */}
        {routeOverlay}
      </DeckGL>
